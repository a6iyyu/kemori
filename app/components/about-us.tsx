...
export const AboutUs: React.FC = () => {
  return (
    <>
      <h3
        className="h-fit w-fit border-2 border-solid cursor-default font-semibold text-sm lg:text-lg text-center mb-8 mx-auto px-8 py-3 rounded-full"
        id="about-section"
      >
      ...
      <section className="h-fit w-4/5 flex flex-col xl:flex-row items-center justify-between mt-1 xl:mt-0 mx-auto">
        {narrative.map((reason) => (
          <div
            key={reason.id}
            className="h-[23rem] md:h-72 xl:h-96 w-full xl:w-1/3 bg-gradient-to-tl from-slate-50 via-slate-200 to-slate-100 rounded-3xl first:mr-0 first:mt-8 xl:first:mr-5 even:mx-0 even:mt-8 lg:even:mx-5 last:ml-0 last:mt-8 lg:last:ml-5"
            style={{ boxShadow: "0 0 1.25rem #bcbcbc" }}
          >
            <img
              src={reason.image}
              alt={reason.alt}
              ...
              id="narrative-icon"
            />
            <p className="h-fit w-4/5 text-lg cursor-default font-semibold mx-auto mt-4">
              {reason.title}
            </p>
            <p className="h-fit w-4/5 text-md cursor-default font-medium text-justify mx-auto mt-2.5 mb-8">
              {reason.description}
            </p>
          </div>
        ))}
